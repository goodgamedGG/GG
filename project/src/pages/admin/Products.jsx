...
const Products = () => {
    ...
    const handleBulkDelete = async () => {
        ...
        if (window.confirm(`Are you sure you want to delete ${selectedProducts.length} products?`)) {
            try {
                await adminAPI.bulkDeleteProducts({ productIds: selectedProducts });
                setSelectedProducts([]);
                loadData();
    ...
    const handleBulkUpdate = async () => {
        ...
        try {
            await adminAPI.bulkUpdateProducts({ productIds: selectedProducts, updates });
            setSelectedProducts([]);
            setBulkUpdates({ isActive: '', isFeatured: '' });
            loadData();
            setIsBulkModalOpen(false);
        } catch (error) {
    ...
    const handleSaveTags = async () => {
        try {
            await adminAPI.updateProductTags(editingTagsProduct._id, tagsData);
